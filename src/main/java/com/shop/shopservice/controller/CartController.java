...
import com.shop.shopservice.serviceimpl.UserAddressServiceImpl;
...
public class CartController {
	...
	ResponseEntity<Map<String, String>> createCart(@Valid @RequestBody Map<String, String> json,
		...
		if (null != json.get(ServiceConstants.USER_ID) && null != json.get(ServiceConstants.SHOP_ID)
			...
			float gstAmount = 0, discount = 0, price = 0, totalAmount = 0, deliveryCharge = 0, productPrice = 0;
			...
			if (product.getStock() >= productQuantity) {
				...
				if (result) {
					...
					price = calculation.DecimalCalculation(product.getPrice() * productQuantity);
					gstAmount = calculation.DecimalCalculation(product.getGstAmount() * productQuantity);
					discount = calculation.DecimalCalculation(product.getDiscount() * productQuantity);
					totalAmount = calculation.DecimalCalculation((productPrice * productQuantity) + gstAmount);
	...
	public ResponseEntity<Map<String, String>> getCartByCartIdAndProductId(@PathVariable("cartId") int cartId,
		...
		Calculation calculation = new Calculation();
		float price = product.getPrice(), gstAmount = product.getGstAmount(), discount = product.getDiscount(),
				deliveryCharge = 0, totalAmount = 0;
		if (product.getStock() > productList.getProductQuantity()) {
			totalAmount = calculation.DecimalCalculation(price + gstAmount);
			int payableAmount = (int) Math.ceil(cart.getTotalAmount() + totalAmount);
			...
			if (updateCart) {
				...
				productList.setPrice(calculation.DecimalCalculation(productList.getPrice() + price));
				...
				productList
						.setOldPrice(calculation.DecimalCalculation(productList.getOldPrice() + product.getOldPrice()));
	...
	public ResponseEntity<Map<String, String>> cartDecrease(@PathVariable("cartId") int cartId,
		...
		float price = product.getPrice(), gstAmount = product.getGstAmount(), totalAmount = 0,
				discount = product.getDiscount(), deliveryCharge = product.getDeliveryCharge();
		int payableAmount = 0;
	...
	ResponseEntity<Map<String, String>> razorPayOrder(@Valid @RequestBody Map<String, String> json,
		...
		//	RazorpayClient razorpay = new RazorpayClient("[YOUR_KEY_ID]", "[YOUR_KEY_SECRET]");
			...
			orderRequest.put("orderId", order.get("id").toString());
			
			response.put("orderId", order.get("id"));
			response.put("currency", ServiceConstants.DEFAULT_ORDER_CURR);
...
//	@PutMapping("placeorder1")
//	ResponseEntity<Map<String, String>> placeOrder1(@Valid @RequestBody Map<String, String> json,
//			HttpServletRequest request) throws URISyntaxException {
//		Map<String, String> response = new HashMap<String, String>();
//		if (null != json.get(ServiceConstants.ORDER_STATUS) && null != json.get(ServiceConstants.CART_ID)
//				&& null != json.get(ServiceConstants.ADDRESS_ID)
//				&& null != json.get(ServiceConstants.TRANSACTION_TYPE)) {
//			int orderStatus = Integer.parseInt(json.get(ServiceConstants.ORDER_STATUS)),
//					cartId = Integer.parseInt(json.get(ServiceConstants.CART_ID)),
...
//			LookUp lookUp3 = lookup.findLookUpIdByName("MILAAN", "CUSTOMER");
//			LookUp lookUp4 = lookup.findLookUpIdByName("MILAAN", "TRANS_PENDING");
//			LookUp lookUp5 = lookup.findLookUpIdByName("MILAAN", "ORDER_NOTIFICATION");
//			int place = lookUp.getLookUpId();
//			int online = lookUp1.getLookUpId();
//			int cash = lookUp2.getLookUpId();
//			int customer = lookUp3.getLookUpId();
//			int transactionPending = lookUp4.getLookUpId();
//			int orderNotification = lookUp5.getLookUpId();
//
//			Cart cart = cartService.getCart(cartId);
//			String shopId = cart.getShopId();
//			int userId = Integer.parseInt(cart.getUserId());
...
//			String phonNumber = address.getMobileNo();
//			String order_rcptid_11 = Strings.EMPTY;
//			order_rcptid_11 = UUID.randomUUID().toString().substring(0, 10);
//			Order order = null;
//			try {
...
//				orderRequest.put("receipt", order_rcptid_11);
//				orderRequest.put("payment_capture", Boolean.FALSE);
//				order = new RazorpayClient(ServiceConstants.RAZORPAY_KEY, ServiceConstants.RAZORPAY_SECRET).Orders
//						.create(orderRequest);
//				System.out.println(String.valueOf("Order Id" + order.get("id")));
//
//				response.put("orderId", order.get("id"));
...
//
//					transaction.setActive(Boolean.TRUE);
//					transaction.setAdminId(Integer.parseInt(cart.getAdminId()));
...
//					transaction.setTransactionStatus(transactionPending);
//					transaction.setUserId(userId);
...
//					} else {
//						response.put("Status", Boolean.FALSE.toString());
//						response.put("Descreption", "Productlist is empty");
//					}
//
//				}
//				boolean result = cartService.updateCart(cart);
...
//		} else {
//			response.put("Status", Boolean.FALSE.toString());
//			response.put("Descreption", " please give all data");
//		}
//		return ResponseEntity.ok().body(response);
//	}
//
...
//					response.put("description", "Your order received sucessfully.");
	...
	public ResponseEntity<List<Cart>> getCartByShopIdUserId(@PathVariable("shopId") String shopId,
...
//	ResponseEntity<Map<String, String>> createOfflineCart(@Valid @RequestBody Map<String, String> json,
//			HttpServletRequest request) throws URISyntaxException {
//		log.info("Request to create user: {}", json.get(ServiceConstants.USER_NAME));
//		Map<String, String> response = new HashMap<String, String>();
//		if (null != json.get(ServiceConstants.USER_NAME) && null != json.get(ServiceConstants.SHOP_ID)
//				&& null != json.get(ServiceConstants.PRODUCT_ID) && null != json.get(ServiceConstants.PRODUCT_QUANTITY)
//				&& null != json.get(ServiceConstants.MOBILE_NUMBER)) {
//			String shopId = json.get(ServiceConstants.SHOP_ID);
//
//			Cart cart = null;
//			List<Cart> cartList = null;
...
//			int orderStatus = lookup1.getLookUpId();
//
//			String userName = json.get(ServiceConstants.USER_NAME), productId = json.get(ServiceConstants.PRODUCT_ID);
//
//			product = productService.getProduct(Integer.parseInt(productId));
//			int productQuantity = Integer.parseInt(json.get(ServiceConstants.PRODUCT_QUANTITY));
//
//			boolean result = cartService.offlinecartExists(shopId, orderStatus);
//			if (result) {
//				cartList = cartService.getOfflineCart(shopId, orderStatus);
//				cart = cartList.get(0);
//				int totalAmount = product.getPrice() * productQuantity,
...
//
//				if (finalResult) {
//					result = cartService.offlinecartExists(shopId, orderStatus);
//					if (result) {
//						// cartList = cartService.getCartByUserId(userId, shopId, orderStatus);
//						cartList = cartService.getOfflineCart(shopId, orderStatus);
//						cart = cartList.get(0);
